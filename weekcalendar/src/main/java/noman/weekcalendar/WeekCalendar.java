...
import noman.weekcalendar.listener.OnMonthChangeListener;
...
public class WeekCalendar extends LinearLayout {
    ...
    @Subscribe
    public void onMonthChange(Event.OnMonthChange event) {
        if (onMonthChangeListener != null) {
            onMonthChangeListener.onMonthChange(event.getFirstDayOfTheWeek(), event.isForward());
        }
    }
...
//                daysName.add(daysName.remove(0));
                ...
                if (typedArray.getInt(R.styleable.WeekCalendar_dayNameLength, 0) == 0) {
                    for (int i = 0; i < daysName.size(); i++) {
                        daysName.set(i, daysName.get(i).substring(1));
                    }
                }
    ...
    public void lastMonth(){
        BusProvider.getInstance().post(new Event.LastMonthEvent());
    }

    public void nextMonth(){
        BusProvider.getInstance().post(new Event.NextMonthEvent());
    }
